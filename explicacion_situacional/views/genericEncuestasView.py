        ...
        for pregunta in Pregunta.objects.filter(consulta_id=kwargs['pk']).all():
            ...
            if pregunta.tipo_pregunta.id == TIPOS_PREGUNTAS['SELECT_SIMPLE']:
                ...
                if(pregunta.tipo_pregunta.id == TIPOS_PREGUNTAS['SI_NO']):
                    ...
                    campo += '<label for="'+kwargs['pk']+'">No</label><input type="radio" name="consulta_respuesta_sino_'+str(pregunta.id)+'" id="'+kwargs['pk']+'"value="No" class="icheck need_justification">'
                    campo += '<div id="div_justificar_'+kwargs['pk']+'" style="display:none;"><label>Justifique su Respuesta</label>'
                    campo += '<textarea rows="20" cols="50" class="form-control" id="respuesta_justificar_'+kwargs['pk']+'" name="consulta_respuesta_justificar_'+str(pregunta.id)+'">'
                    campo += '</textarea></div>'
